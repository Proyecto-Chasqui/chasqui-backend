<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>
Bienvenido a Chasqui
</title>
</head>
<body id="cuerpo">
<table width="600" align="center" cellpadding="0" cellspacing="0" border="0">
<tbody>

<!-- Logo Chasqui -->
<tr style="height: 90px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; height: 90px;"><img src="Cid:logochasqui" border="0" alt="Chasqui" /></td>
</tr>
<!-- Imagen de referencia -->
<tr style="height: 130px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: center; height: 130px;"><img src="cid:pedidopreparado" width="100%" border="0" alt="Comunidad de consumo solidario" /></td>
</tr>

<!-- cuerpo del mail -->

<tr style="height: 20px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 20px;">&nbsp;</td>
</tr>
<tr style="height: 15px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 15px;">&nbsp;</td>
</tr>

<tr style="height: 40px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 40px;">&nbsp;</td>
</tr>

<tr style="height: 21px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 18px; color: #777777; text-align: center; height: 21px;">${textoDetalle}</td>
</tr>
<tr style="height: 40px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 40px;">&nbsp;</td>
</tr>

<tr style="height: 21px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 18px; color: #777777; text-align: center; height: 21px;">${tablaContenidoPedido}</td>
</tr>
<tr style="height: 40px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 40px;">&nbsp;</td>
</tr>

<tr style="height: 21px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 18px; color: #777777; text-align: center; font-weight: bold; height: 21px;">${tablaDireccionEntrega}</td>
</tr>
<tr style="height: 40px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 40px;">&nbsp;</td>
</tr>

<tr style="height: 21px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 18px; color: #777777; text-align: center; height: 21px;">${sugerencia}</td>
</tr>

<tr style="height: 21px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 18px; color: #777777; text-align: center; height: 21px;">Gracias por seguir participando en la construcción de un mundo mejor!</td>
</tr>


<tr style="height: 40px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 40px;">&nbsp;</td>
</tr>
<tr style="height: 40px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 40px;">&nbsp;</td>
</tr>

<tr style="height: 21px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 12px; color: #777777; text-align: center; height: 14px;">ATENCIÓN: Este es un mensaje generado automáticamente por el Sistema Chasqui y no debe ser respondido.</td>
</tr>


<tr style="height: 40px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 40px;">&nbsp;</td>
</tr>
<tr style="height: 20px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 18px; color: #777777; text-align: center; height: 20px;">&iquest;Queres saber más de Chasqui? Ingresá a:</td>
</tr>
<tr style="height: 40px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 32px; color: #777777; text-align: center; font-weight: bold; height: 40px;"><a href="http://observatorioess.org.ar/chasqui">OBSERVATORIO DE LA ECONOMÍA SOCIAL Y SOLIDARIA</a></td>
</tr>

<!-- footer linia violeta -->
<tr style="height: 50px;">
<td align="center" style="margin: 0px; padding: 0px; border: 0px; height: 50px;">
<table width="600" align="center" cellpadding="0" cellspacing="0" border="0">
<tbody>
<tr>
<td style="margin: 0; padding: 0; border: 0; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; border-bottom: #10994a 7px solid;">&nbsp;</td>
</tr>
<tr>
<td style="margin: 0; padding: 0; border: 0; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify;">&nbsp;</td>
</tr>
</tbody>
</table>
</td>
</tr>

<!-- footer -->

<tr style="height: 20px;">
<td style="margin: 0px; padding: 0px; border: 0px; font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #777777; text-align: justify; height: 20px;">&nbsp;</td>
</tr>
</tbody>
</table>
</body>
</html>

