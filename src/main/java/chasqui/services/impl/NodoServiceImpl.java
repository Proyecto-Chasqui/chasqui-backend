...
public class NodoServiceImpl implements NodoService {
	...
	@Override
	public void crearSolicitudDeCreacionNodo(Integer idVendedor, Cliente usuario, String nombre, Direccion direccion,
		...
		solicitudCreacionNodoDAO
				.guardar(new SolicitudCreacionNodo(idVendedor, usuario, nombre, direccion, tipo, barrio, descripcion));
	...
	public void crearSolicitudDePertenenciaANodo(Nodo nodo, Cliente usuario) throws NodoCerradoException {
		...
		notificacionService.notificarSolicitudDePertenenciaANodo(usuario.getEmail(), nodo.getAdministrador().getEmail(),
				usuario.getNombre(), nodo.getAlias());
	}

	@Override
	public List<Nodo> obtenerNodosDelVendedor(Integer idVendedor) throws VendedorInexistenteException {
		usuarioService.obtenerVendedorPorID(idVendedor); // Para verificar si el vendedor con el id pasado como parametro es
																											// válido
	...
	@Deprecated
	@Override
	public void altaNodoSinUsuario(String alias, String emailClienteAdministrador, String localidad, String calle,
			int altura, String telefono, int idVendedor, String descripcion)
	...
	public List<SolicitudCreacionNodo> obtenerSolicitudesDeCreacionDe(String email, Integer idVendedor)
		...
		return solicitudCreacionNodoDAO.obtenerSolicitudesDeCreacionDe(usuarioService.obtenerClientePorEmail(email).getId(),
				idVendedor);
	}
	...
	public void editarSolicitudDeCreacionNodo(Integer idVendedor, Cliente cliente, Integer idSolicitud, String nombreNodo,
		...
		SolicitudCreacionNodo solicitud = solicitudCreacionNodoDAO.obtenerSolitudCreacionNodo(idSolicitud, cliente.getId(),
				idVendedor);
	...
	@Override
	public void cancelarSolicitudDeCreacionNodo(Integer idSolicitud, Integer idVendedor, Integer idCliente)
			throws SolicitudCreacionNodoException, VendedorInexistenteException, ConfiguracionDeVendedorException {
		SolicitudCreacionNodo solicitud = solicitudCreacionNodoDAO.obtenerSolitudCreacionNodo(idSolicitud, idCliente,
				idVendedor);
		validarEstrategiaNodoActiva(idVendedor);
		validarSolicitud(solicitud);
		solicitud.setEstado(Constantes.SOLICITUD_NODO_CANCELADO);
		solicitudCreacionNodoDAO.guardar(solicitud);
		notificacionService.notificarCancelacionDeSolicitudCreacionNodoAVendedor(idVendedor, solicitud.getNombreNodo(),
	...
	private void validarNombreNodo(String nombre, Integer idVendedor) throws NodoYaExistenteException {
		List<SolicitudCreacionNodo> solicitudes = solicitudCreacionNodoDAO
				.obtenerSolicitudesDeCreacionEnGestionDe(idVendedor);
	...
	private void validarNombreDeSolicitudes(List<SolicitudCreacionNodo> solicitudes, String nombre)
	...
	private void validarNombreNodoAlEditar(String nombreNodo, Integer idVendedor, Integer idSolicitud)
			throws NodoYaExistenteException {
		List<SolicitudCreacionNodo> solicitudes = solicitudCreacionNodoDAO
				.obtenerSolicitudesDeCreacionEnGestionDe(idVendedor);
		List<Nodo> nodos = nodoDAO.obtenerNodosDelVendedor(idVendedor);
	...
	private void validarNombreDeSolicitudesAlEditar(List<SolicitudCreacionNodo> solicitudes, String nombreNodo,
			Integer IdSolicitud) throws NodoYaExistenteException {
		for (SolicitudCreacionNodo solicitud : solicitudes) {
	...
	private void validarEstrategiaNodoActiva(Integer idVendedor)
			throws VendedorInexistenteException, ConfiguracionDeVendedorException {
	...
	private void validar(Cliente usuario, Direccion direccion, Integer idVendedor) throws DireccionesInexistentes,
			VendedorInexistenteException, ConfiguracionDeVendedorException, SolicitudCreacionNodoEnGestionExistenteException {
		if (direccion == null) {
	...
	public void vaciarNodo(Integer idNodo) throws EstadoPedidoIncorrectoException {
		...
		if (nodo.sePuedeEliminar()) {
			...
			throw new EstadoPedidoIncorrectoException(
					"El grupo no puede ser eliminado, por que hay pedidos abiertos o confirmados");
		}
	...
	public void cederAdministracion(Integer idNodo, String emailCliente)
		...
		Nodo nodo = nodoDAO.obtenerNodoPorId(idNodo);
		Cliente administradorAnterior = nodo.getAdministrador(); // Es necesario guardar la referencia para notificarlo
																															// luego que cedio la administracion.
	...
	public void quitarMiembroDelNodo(Integer idNodo, String emailCliente) throws UsuarioInexistenteException {
		...
		try {
			...
			SolicitudPertenenciaNodo solicitud = solicitudPertenenciaNodoDAO.obtenerSolicitudDe(nodo.getId(),
					cliente.getId());
			if (solicitud != null) {
			...
			// Si el usuario no existe es porque fue invitado pero no está registrado en
			// chasqui
	...
	@Override
	public void invitarANodo(Integer idNodo, String emailInvitado, String emailAdministrador)
		...
		if (nodo.fueInvitado(emailInvitado)) {
			throw new GrupoCCInexistenteException(
					"El usuario que intenta invitar ya fue invitado al grupo o ya pertence al grupo");
		}

		try {
			Cliente cliente = (Cliente) usuarioService.obtenerUsuarioPorEmail(emailInvitado);

			notificacionService.notificarInvitacionAGCCClienteRegistrado(administrador, emailInvitado, nodo,
					cliente.getIdDispositivo());
		...
		if (nodo.getAdministrador().getEmail().equals(email)) {
			...
			if (direccion == null) {
				throw new RequestIncorrectoException(
						"El usuario " + nodo.getAdministrador().getNombre() + " no posee la dirección asignada");
			}
			...
			throw new RequestIncorrectoException(
					"El usuario " + email + " no es el administrador del grupo:" + nodo.getAlias());
		}
	...
	public void aceptarSolicitudDePertenencia(SolicitudPertenenciaNodo solicitudpertenencia) {
		...
		notificacionService.notificarGestionDeSolicitudDePertenenciaANodo("acepto",
				solicitudpertenencia.getUsuarioSolicitante().getEmail(), nodo.getAdministrador().getEmail(), nodo.getAlias());
	}
	...
	public void rechazarSolicitudDePertenencia(SolicitudPertenenciaNodo solicitudpertenencia) {
		...
		solicitudPertenenciaNodoDAO.guardar(solicitudpertenencia);
		notificacionService.notificarGestionDeSolicitudDePertenencia(solicitudpertenencia);
		notificacionService.notificarGestionDeSolicitudDePertenenciaANodo("rechazo",
				solicitudpertenencia.getUsuarioSolicitante().getEmail(), nodo.getAdministrador().getEmail(), nodo.getAlias());
	}
	...
	public void reabrirSolicitudDePertenenciaNodo(SolicitudPertenenciaNodo solicitud) {
		...
		solicitud.setReintentos(solicitud.getReintentos() + 1);
		solicitudPertenenciaNodoDAO.guardar(solicitud);
		notificacionService.enviarEmailDeSolicitudDePertenenciaANodo(solicitud.getNodo(),
				(Cliente) solicitud.getUsuarioSolicitante());
		notificacionService.notificarSolicitudDePertenenciaANodo(solicitud.getUsuarioSolicitante().getEmail(),
				solicitud.getNodo().getAdministrador().getEmail(), solicitud.getUsuarioSolicitante().getUsername(),
				solicitud.getNodo().getAlias());
	}

	private List<MiembroDeGCC> obtenerOtrosMiembrosDelNodo(String mailCliente, Integer idNodo)
	...
	private boolean pedidoEnEstadoInactivo(Pedido pedido) {
		if (pedido != null) {
			return pedido.getEstado().equals(Constantes.ESTADO_PEDIDO_CANCELADO)
					|| pedido.getEstado().equals(Constantes.ESTADO_PEDIDO_VENCIDO)
					|| pedido.getEstado().equals(Constantes.ESTADO_PEDIDO_INEXISTENTE);
		} else {
			return true;
		}
	}

	private void notificarNuevoPedidoIndividualAOtrosMiembros(GrupoCC grupo, String emailCliente, String nombreCliente,
			String nombreVendedor) throws UsuarioInexistenteException, ClienteNoPerteneceAGCCException,
			GrupoCCInexistenteException, VendedorInexistenteException, PedidoInexistenteException {
		...
		notificacionService.notificarNuevoPedidoEnGCC(grupo.getId(), grupo.getAlias(), emailCliente,
				grupo.getAdministrador().getEmail(), nombreCliente, nombreVendedor);
		/*
		 * Se comenta esta sección debido a que es muy agresiva la notificación en
		 * situaciones de muchos integrantes. for (MiembroDeGCC compa : compas) {
		 ...
		 * { if(compa.getEstadoInvitacion().equals(Constantes.
		 * ESTADO_NOTIFICACION_LEIDA_ACEPTADA)) {
		 * notificacionService.notificarNuevoPedidoEnGCC(grupo.getId(),grupo.getAlias(),
		 * emailCliente, compa.getEmail(), nombreCliente, nombreVendedor); } } }else {
		 * if(compa.getEstadoInvitacion().equals(Constantes.
		 * ESTADO_NOTIFICACION_LEIDA_ACEPTADA)) {
		 * notificacionService.notificarNuevoPedidoEnGCC(grupo.getId(),grupo.getAlias(),
		 * emailCliente, compa.getEmail(), nombreCliente, nombreVendedor); } } }
		 */
	}
	...
	public void nuevoPedidoIndividualPara(Integer idNodo, String email, Integer idVendedor)
		...
		if (nodo.pertenece(email)) {
			if (pedidoVigente == null) {
				...
				if (pedidoVigente.getEstado().equals(Constantes.ESTADO_PEDIDO_CANCELADO)
						|| pedidoVigente.getEstado().equals(Constantes.ESTADO_PEDIDO_VENCIDO)) {
	...
	private void validarQueContengaProductos(Pedido pedido) throws PedidoSinProductosException {
		if (pedido.getProductosEnPedido().isEmpty()) {
			throw new PedidoSinProductosException(
					"El usuario: " + pedido.getCliente().getUsername() + " no posee productos en su pedido");
		}
	}

	/*
	 * Este método notifica a los miembros del nodo que un cliente ha confirmado el
	 * pedido individual
	 * 
	 */
	private void notificarConfirmacionAOtrosMiembros(String emailCliente, String nombreCliente, Pedido pedido,
			Integer idVendedor, String emailVendedor) throws UsuarioInexistenteException, ClienteNoPerteneceAGCCException,
			GrupoCCInexistenteException, VendedorInexistenteException, PedidoInexistenteException {
		...
		// Solo se notifica al coordinador del nodo, eliminar o comentar si la
		// funcionalidad a todos se habilita, ya que le llegaran 2 notificaciones.
		notificacionService.notificarConfirmacionNodoCompraOtroMiembro(emailVendedor, nodo.getAdministrador().getEmail(),
				nombreCliente, nodo.getAlias());
		/*
		 * rehabilitar si esta funcionalidad es requerida, por el momento los nodos
		 * poseen tanta cantidad de usuarios que es muy molesto for (MiembroDeGCC compa
		 * : compas) { if(compa.getEstadoInvitacion().equals(Constantes.
		 * ESTADO_NOTIFICACION_LEIDA_ACEPTADA)) {
		 * notificacionService.notificarConfirmacionNodoCompraOtroMiembro(emailVendedor,
		 * compa.getEmail(),nombreCliente , nodo.getAlias()); } }
		 */
	}
	...
	private void validarConfirmacionDePedidoSinDireccionPara(Pedido p, ConfirmarPedidoSinDireccionRequest request)
		...
		if (p == null) {
			throw new PedidoInexistenteException("El usuario no posee un pedido vigente con el ID otorgado");
		}
	}

	@Override
	public void confirmarPedidoIndividualEnNodo(String email, ConfirmarPedidoSinDireccionRequest request)
		...
		this.notificarConfirmacionAOtrosMiembros(email, cliente.getUsername(), pedido, vendedor.getId(),
				vendedor.getEmail());
	...
	public List<Nodo> obtenerNodosDelVendedorCon(Integer idvendedor, Date d, Date h, String estadoNodoBool,
		...
		return nodoDAO.obtenerNodosDelVendedorCon(idvendedor, d, h, estadoNodoBool, nombreNodo, emailcoordinador, barrio,
				tipo, idZona);
	}
	...
	public Collection<? extends SolicitudCreacionNodo> obtenerSolicitudesDeCreacionNodosDelVendedorCon(Integer id, Date d,
		...
		return solicitudCreacionNodoDAO.obtenerSolicitudesDeCreacionNodosDelVendedorCon(id, d, h, estado, nombreCoordinador,
				email, barrio);
	}
	...
	@Override
	public void eliminarSolicitudesDePertenenciaANodo(List<SolicitudPertenenciaNodo> solicitudesDePertenenciaDeVendedor) {
