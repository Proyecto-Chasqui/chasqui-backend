...
public class CaracteristicaComposer extends GenericForwardComposer<Component>{
	...
	private void validarCaracteristica(String nombre, String descripcion, Imagen img){
		if(StringUtils.isEmpty(nombre)){
			throw new WrongValueException(txtbCaracteristicaProductor,"El nombre de la caracteristica no debe ser vacia");
		}
		if(img == null){
	...
	/*
	 * Evento disparado por la selección de una imagen de una caracteristica del Productor
	 */
	...
	/*
	 * Evento disparado por la selección de una imagen de una caracteristica del Producto
	 */
	...
	//---------------------------------Metodos para guardar imagenes y recuperarlas del Filesystem
	...
	public void actualizarImagenProducto(UploadEvent evt){
		try{
			...
			imagen = fileSaver.guardarImagen(getAbsolutePath(),usuario.getUsername(),image.getContent().getName(),image.getContent().getByteData());			
	...
	public void actualizarImagenProductor(UploadEvent evt){
		try{
			Image image = cargarImagen(evt);
			imagenProductor = fileSaver.guardarImagen(getAbsolutePath(),usuario.getUsername(),image.getContent().getName(),image.getContent().getByteData());
	...
	public void eliminarCaracteristica(ICaracteristica c, String tipo){
		Messagebox.show("Está seguro de eliminar la caracteristica seleccionada??", "Advertencia", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION,new DialogoBorrar(tipo,c, this));
	}
	...
	public void editarCaracteristica(ICaracteristica c, String tipo) {	
		...
		if (tipo.equals(CaracteristicaRenderer.CTE_PRODUCTO)) {
		    ...
		    this.caracteristicasProductor.remove(c);
		}
	}

	/*
	 * Carga los datos de la caracteristica del PRODUCTOR que fue seleccionada para editarse
	 */
	...
	/*
	 * Carga los datos de la caracteristica del PRODUCTO que fue seleccionada para editarse
	 */
