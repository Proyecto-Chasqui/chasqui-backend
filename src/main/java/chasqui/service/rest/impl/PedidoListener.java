...
public class PedidoListener {
	...
	private void validarSiUsaEstrategiaIndividual(Integer idVendedor)
	...
	@Path("/{idPedido : \\d+ }/productos")
	public Response obtenerProductosDePedido(@PathParam("idPedido") final Integer idPedido) {
		...
		try {
			...
			return Response.ok(productoPedidoService.obtenerLite(query), MediaType.APPLICATION_JSON).build();
		} catch (Exception e) {
	...
	public Response confirmarPedidoEnGrupo(
	...
	@POST
	/**
	 * Refresca la fecha de vencimiento del pedido con el id que recibe en el
	 * idRequest. Precondiciones: El pedido pertenece al usuario logeado. El pedido
	 * tiene que estar con estado ABIERTO.
	 * 
	 */
	@Produces("application/json")
	@Path("/refrescarVencimiento")
	public Response refrescarVencimiento(
			@Multipart(value = "idRequest", type = "application/json") final String idRequest) {
	...
	public Response obtenerOCrearPedidoIndividualParaUsuario(
			@Multipart(value = "crearRequest", type = "application/json") final String crearRequest) {
		String mail = obtenerEmailDeContextoDeSeguridad();
		...
		try {
			...
			return Response.ok(toResponse(pedidoService.obtenerPedidoActualDe(mail, idVendedor)), MediaType.APPLICATION_JSON)
					.build();
		} catch (PedidoInexistenteException e) {
			try {
				pedidoService.crearPedidoIndividualPara(mail, idVendedor);
				return Response
						.ok(toResponse(pedidoService.obtenerPedidoActualDe(mail, idVendedor)), MediaType.APPLICATION_JSON).build();
	 ...
	 * crearPedidoIndividualParaUsuario obtenerPedidoActual obteniendo el pedido
	 * actual del usuario para el vendedor solicitado y si este no existiera lo
	 * generara y retornara. Por eso se encuentra deprecado 12/04/2018
	 */
	@POST
	@Produces("application/json")
	@Path("/individual")
	public Response crearPedidoIndividualParaUsuario(
			@Multipart(value = "crearRequest", type = "application/json") final String crearRequest) {
		String mail = obtenerEmailDeContextoDeSeguridad();
		try {
	...
	public Response obtenerPedidosVigentes() {
		String mail = obtenerEmailDeContextoDeSeguridad();
		try {
			return Response.ok(toListResponse(pedidoService.obtenerPedidosVigentesEnTodosLosCatalogosPara(mail)),
	...
	public Response agregarProductoAPedido(
			@Multipart(value = "agregarRequest", type = "application/json") final String agregarRequest) {
		try {
	...
	private void validarEstrategiasVendedor(Integer idVendedor)
			throws ConfiguracionDeVendedorException, VendedorInexistenteException {
		...
		if (!vendedor.isVentasHabilitadas()) {
			throw new ConfiguracionDeVendedorException(
					"El vendedor por el momento no permite hacer compras o agregar mas productos, intentelo mas tarde.");
		}
		;
	}
	...
	public Response eliminarProductoDelPedido(
			@Multipart(value = "eliminarRequest", type = "application/json") final String eliminarRequest) {
		try {
	...
	@Produces("application/json")
	@Path("/conEstados")
	public Response obtenerPedidosConEstado(
			@Multipart(value = "obtenerPedidosConEstadoRequest", type = "application/json") final String obtenerPedidosConEstadoRequest)
			throws JsonParseException, JsonMappingException, IOException {
		ObtenerPedidosConEstadoRequest request = toObtenerPedidosConEstadoRequest(obtenerPedidosConEstadoRequest);
		String mail = obtenerEmailDeContextoDeSeguridad();
		try {
			return Response.ok(
					toListResponse(pedidoService.obtenerPedidosConEstados(mail, request.getIdVendedor(), request.getEstados())),
					MediaType.APPLICATION_JSON).build();
		} catch (Exception e) {
			return Response.status(500).entity(new ChasquiError(e.getMessage())).build();
		}
	...
	public Response obtenerPedidosColectivosConEstado(
			@Multipart(value = "pedidosColectivosConEstados", type = "application/json") final String obtenerPedidosColectivosConEstadoRequest)
			throws JsonParseException, JsonMappingException, IOException {
		ObtenerPedidosColectivosConEstadoRequest request = toObtenerPedidosColectivosConEstadoRequest(
				obtenerPedidosColectivosConEstadoRequest);
		String mail = obtenerEmailDeContextoDeSeguridad();
		try {
			Usuario u = usuarioService.obtenerClientePorEmail(mail);
			return Response.ok(toListPCResponse(pedidoColectivoService.obtenerPedidosColectivosDeGrupoConEstado(u.getId(),
					request.getIdGrupo(), request.getEstados())), MediaType.APPLICATION_JSON).build();
		} catch (Exception e) {
			return Response.status(500).entity(new ChasquiError(e.getMessage())).build();
		}
	}
	...
	private ObtenerPedidosConEstadoRequest toObtenerPedidosConEstadoRequest(String obtenerPedidosConEstadoRequest)
			throws JsonParseException, JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper();
		mapper.configure(JsonParser.Feature.ALLOW_UNQUOTED_FIELD_NAMES, true);
	...
	private ConfirmarPedidoRequest toConfirmarPedidoRequest(String request)
			throws JsonParseException, JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper();
		mapper.configure(JsonParser.Feature.ALLOW_UNQUOTED_FIELD_NAMES, true);
